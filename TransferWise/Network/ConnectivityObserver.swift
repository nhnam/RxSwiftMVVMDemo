...
//  TransferWise
//
//  Created by Nam Nguyen on 21/11/18.
...
public class ConnectivityObserver {
    ...
    public typealias NetworkConnected = (ConnectivityObserver) -> Void
    public typealias NetworkDisconnected = (ConnectivityObserver) -> Void
    ...
    public var successThreshold: ConnectivityObserver.Percentage = ConnectivityObserver.Percentage(75.0)
    
    /// URLs to contact in order to check connectivity
    public var connectivityURLs: [URL] = ConnectivityObserver
        .defaultConnectivityURLs(shouldUseHTTPS: ConnectivityObserver.isHTTPSOnly) {
        didSet {
            if ConnectivityObserver.isHTTPSOnly { // if HTTPS only set only allow HTTPS URLs
...
// Private API
private extension ConnectivityObserver {
